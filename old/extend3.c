...
void mirror_board (int board[BOARD_HEIGHT][BOARD_WIDTH], int mirrored_board[BOARD_HEIGHT][BOARD_WIDTH]);
...
int array_height_coordinate (int height_coordinate){
    ...
    array_height_coordinate = ((0 - height_coordinate) + (BOARD_HEIGHT));
...
//generates random number for each unique board
int allocate_board_identifier (int board[BOARD_HEIGHT][BOARD_WIDTH]){
    int x_val = 1;
    int y_val = 2;
    for (int i = 0; i < BOARD_HEIGHT; ++i){
